...
type Availability = {
  startTime: string
  endTime: string
  dayOfWeek: (typeof DAYS_OF_WEEK_ORDERED)[number]
}
...
export function ScheduleForm({
  ...
  const form = useForm<z.infer<typeof scheduleFormSchema>>({
    ...
    defaultValues: {
      ...
      availabilities: schedule?.availabilities.toSorted((a, b) => {
        return timeToInteger(a.startTime) - timeToInteger(b.startTime)
      }),
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof scheduleFormSchema>) {
    const data = await saveSchedule(values, user);
    ...
    if (data?.error) {
      ...
      setSuccessMessage("Availability schedule saved!")
    }
  }
    ...
    <Form {...form}>
      ...
      >
        ...
        <div className="grid grid-cols-[auto,1fr] gap-y-6 gap-x-4">
          {DAYS_OF_WEEK_ORDERED.map(dayOfWeek => (
